-- ============================================================
-- SCHEMA ĐẦY ĐỦ — ĐÃ SẮP XẾP ĐÚNG THỨ TỰ + TÍCH HỢP CÁC FIX
...
-- ── 4. BẢNG PHỤ THUỘC VÀO users + licenses ───────────────────
-- [FIX 1] license_key nullable, bỏ FK tới licenses
-- [FIX 2] thêm cột pending_earned

CREATE TABLE public.affiliates (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  license_key text,                          -- FIX: nullable, bỏ NOT NULL
  name text NOT NULL,
  phone text NOT NULL,
  email text,
  bank_name text,
  ...
  pending_earned integer DEFAULT 0,          -- FIX: cột bị thiếu
  status text DEFAULT 'active'::text,
  created_at timestamp with time zone DEFAULT now(),
  user_id uuid,
  CONSTRAINT affiliates_pkey PRIMARY KEY (id),
  CONSTRAINT affiliates_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id)
  -- FIX: bỏ affiliates_license_key_fkey
);

-- [FIX 3] license_key nullable
...
-- ── 6. BẢNG PHỤ THUỘC VÀO affiliates + users + orders ────────
