...
async function sendRuleNotification(sb, userId, ruleName, affectedItems) {
  try {
    ...
    const lines = affectedItems.slice(0, 10).map(a => {
      ...
      return `  • ${a.adset_name} | Chi: ${spend} | CPA: ${a.metrics?.cpa ? Number(a.metrics.cpa).toLocaleString('vi-VN') + '₫' : '—'} | ROAS: ${a.metrics?.roas || '—'}`
    }).join('\n')
    const more = affectedItems.length > 10 ? `\n  ... và ${affectedItems.length - 10} mục khác` : ''
    const time = new Date().toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' })
    const msg = `🔔 <b>Cảnh báo Quy tắc tự động (Cron)</b>\n📋 Rule: <b>${ruleName}</b>\n⏰ ${time}\n\n⚠️ <b>${affectedItems.length}</b> mục thỏa điều kiện:\n${lines}${more}\n\n🔗 https://adsmeta.gonetwork.vn/dashboard/automated-rules`
