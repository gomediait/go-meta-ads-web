...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'create') {
      const { name, conditions, action: ruleAction, scale_factor, account_id, time_range } = body
      if (!conditions?.length) return res.status(400).json({ error: 'Cần ít nhất 1 điều kiện' })
      const validActions = ['pause', 'scale_budget', 'reduce_budget']
    ...
    if (action === 'update') {
      const { id, name, enabled, conditions, action: ruleAction, scale_factor, account_id, time_range } = body
